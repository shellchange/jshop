...
.liCss {
	...
	<form action="${basepath}/manage/news" method="post" theme="simple" id="form" name="form">
		...
		<table class="table table-bordered">
			<tr>
				<td>标题</td>
				<td><input type="text" value="${e.title!""}" class="input-medium search-query" name="e.title"/></td>
				<#if e.type=="help">
					<td>文章类型</td>
					<td>
			...
			<tr>
				<td colspan="16">
					...
					<a href="news!toAdd.action?e.type=${e.type}" class="btn btn-success">
		...
		<table class="table table-bordered table-hover">
            ...
            <#list pager.list as item>
				<tr>
					...
					<td >${item.id!""}</td>
					<td class="aCss">
					  <a href="news!toEdit.action?e.id=${item.id!""}"
								>${item.title!""}</a>
					</td>
					<#if e.type=="help">
					...
					<td>
						<a href="news!toEdit.action?e.id=${item.id!""}">编辑</a>|
						<#if item.type="help">
